describe("Jasmine", function() {
  ...
  beforeEach(function() {
    this.bootedJasmine = {
      getEnv: jasmine.createSpy("getEnv").and.returnValue({
        ...
        throwOnExpectationFailure: jasmine.createSpy(
  ...
  it("delegates #coreVersion to jasmine-core", function() {
    this.fakeJasmineCore.version = jasmine
      .createSpy("coreVersion")
      .and.returnValue("a version");
    expect(this.testJasmine.coreVersion()).toEqual("a version");
  });

  it("registers a console reporter upon construction", function() {
    spyOn(Jasmine, "ConsoleReporter").and.returnValue({
  ...
  describe("#configureDefaultReporter", function() {
    ...
    it("sets the options on the console reporter", function() {
      ...
      var expectedReporterOptions = Object.keys(reporterOptions).reduce(
      ...
      expect(this.testJasmine.reporter.setOptions).toHaveBeenCalledWith(
        expectedReporterOptions
      );
    });
  ...
  it("adds matchers to the jasmine env", function() {
    ...
    expect(this.testJasmine.env.addMatchers).toHaveBeenCalledWith([
      "fake matcher 1",
      "fake matcher 2"
    ]);
  });
  ...
  describe("loading configurations", function() {
    ...
    describe("from an object", function() {
      beforeEach(function() {
        this.configObject = {
          spec_dir: "spec",
          spec_files: ["fixture_spec.js", "**/*spec.js"],
          helpers: ["helper.js"],
      ...
      it("can tell jasmine-core to stop spec on expectation failure", function() {
        ...
        expect(
          this.fixtureJasmine.env.throwOnExpectationFailure
      ...
      it("can tell jasmine-core to stop execution when a spec fails", function() {
        ...
        expect(this.fixtureJasmine.env.stopOnSpecFailure).toHaveBeenCalledWith(
          true
        );
      });
      ...
      it("does not configure jasmine-core for stopping execution by default", function() {
        ...
        expect(
          this.fixtureJasmine.env.stopOnSpecFailure
        ).not.toHaveBeenCalled();
      });
      ...
      it("can tell jasmine-core to run random specs", function() {
        ...
        expect(this.fixtureJasmine.env.randomizeTests).toHaveBeenCalledWith(
          true
        );
      });
    ...
    describe("from a file", function() {
      it("adds unique specs to the jasmine runner", function() {
        this.fixtureJasmine.loadConfigFile(
          "spec/support/jasmine_alternate.json"
        );
        expect(this.fixtureJasmine.helperFiles).toEqual([
          "spec/fixtures/sample_project/spec/helper.js"
        ]);
      ...
      it("loads the specified configuration file from an absolute path", function() {
        var absoluteConfigPath = path.join(
          __dirname,
          "fixtures/sample_project/spec/support/jasmine_alternate.json"
        );
      ...
      it("throw error if specified configuration file doesn't exist", function() {
        ...
        function load() {
          jasmine.loadConfigFile("missing.json");
        }
        expect(load).toThrow();
      });
  ...
  describe("#stopSpecOnExpectationFailure", function() {
    it("sets the throwOnExpectationFailure value on the jasmine-core env", function() {
      ...
      expect(
        this.testJasmine.env.throwOnExpectationFailure
  ...
  describe("#stopOnSpecFailure", function() {
    it("sets the stopOnSpecFailure value on the jasmine-core env", function() {
      ...
      expect(this.testJasmine.env.stopOnSpecFailure).toHaveBeenCalledWith(
        "blah"
      );
    });
  });
  ...
  describe("#randomizeTests", function() {
    it("sets the randomizeTests value on the jasmine-core env", function() {
      ...
      expect(this.testJasmine.env.randomizeTests).toHaveBeenCalledWith(
        "foobar"
      );
    });
  });
  ...
  describe("#onComplete", function() {
    it("stores an onComplete function", function() {
      ...
      expect(
        this.testJasmine.completionReporter.onComplete
      ).toHaveBeenCalledWith(fakeOnCompleteCallback);
    });
  });
  ...
  describe("#execute", function() {
    ...
    it("loads helper files before checking if any reporters were added", function() {
      ...
      spyOn(this.testJasmine, "configureDefaultReporter").and.callFake(
        function() {
    ...
    it("adds an exit code reporter", function() {
      var completionReporterSpy = jasmine.createSpyObj("reporter", [
        "onComplete"
      ]);
      ...
      expect(this.testJasmine.addReporter).toHaveBeenCalledWith(
