...
describe("command", function() {
  beforeEach(function() {
    ...
    this.fakeJasmine = jasmine.createSpyObj("jasmine", [
      "loadConfigFile",
      "addHelperFiles",
      "addRequires",
      "showColors",
  ...
  describe("init", function() {
    ...
    it("creates setup folders and files for specs", function() {
      expect(fs.existsSync(path.join(spec, "support/", "jasmine.json"))).toBe(
        true
      );
    });

    it("writes default settings to jasmine.json", function() {
      var realJson = fs.readFileSync(
        path.join(spec, "support/", "jasmine.json"),
        "utf-8"
      );
      var fixtureJson = fs.readFileSync(
        path.join(__dirname, "../", "lib/", "examples/", "jasmine.json"),
        "utf-8"
      );
  ...
  describe("examples", function() {
    ...
    it("should create init files if they do not exist", function() {
      ...
      expect(
        fs.existsSync(path.join(projectBaseDir, "lib", "jasmine_examples"))
      ).toBe(true);
      expect(
        fs.existsSync(path.join(spec, "helpers", "jasmine_examples"))
      ).toBe(true);
    });
    ...
    it("should copy files into the appropriate folder", function() {
      expect(
        fs.existsSync(
          path.join(projectBaseDir, "lib", "jasmine_examples", "Foo.js")
        )
      ).toBe(true);
      expect(
        fs.existsSync(
          path.join(projectBaseDir, "lib", "jasmine_examples", "Bar.js")
        )
      ).toBe(true);
      expect(
        fs.existsSync(path.join(spec, "jasmine_examples", "FooSpec.js"))
      ).toBe(true);
      expect(
        fs.existsSync(
          path.join(spec, "helpers", "jasmine_examples", "SpecHelper.js")
        )
      ).toBe(true);
    });
  });
  ...
  describe("running specs", function() {
    ...
    it("should load a custom config file specified by env variable", function() {
      this.command.run(this.fakeJasmine, [
        "node",
        "bin/jasmine.js",
        "JASMINE_CONFIG_PATH=somewhere.json"
      ]);
      expect(this.fakeJasmine.loadConfigFile).toHaveBeenCalledWith(
        "somewhere.json"
      );
    });
    ...
    it("should be able to run only specified specs", function() {
      this.command.run(this.fakeJasmine, [
        "spec/some/fileSpec.js",
        "SOME_ENV=SOME_VALUE",
        "--no-color"
      ]);
      expect(this.fakeJasmine.execute).toHaveBeenCalledWith(
        ["spec/some/fileSpec.js"],
        undefined
      );
    });
    ...
    it("should be able to add one helper pattern", function() {
      ...
      expect(this.fakeJasmine.addHelperFiles).toHaveBeenCalledWith([
        "helpers/**/*.js"
      ]);
    });
    ...
    it("should be able to add one require", function() {
      ...
      expect(this.fakeJasmine.addRequires).toHaveBeenCalledWith([
        "ts-node/require"
      ]);
    });
    ...
    it("can specify a reporter", function() {
      var reporterPath = path.resolve(
        path.join(__dirname, "fixtures", "customReporter.js")
      );
      ...
      expect(this.fakeJasmine.addReporter).toHaveBeenCalledWith(
        jasmine.any(Reporter)
      );
    });

    it("prints an error if the file does not export a reporter", function() {
      var reporterPath = path.resolve(
        path.join(__dirname, "fixtures", "badReporter.js")
      );
      this.command.run(this.fakeJasmine, ["--reporter=" + reporterPath]);
    ...
    it("prints an error if the reporter file does not exist", function() {
      var reporterPath = path.resolve(
        path.join(__dirname, "fixtures", "missingReporter.js")
      );
      this.command.run(this.fakeJasmine, ["--reporter=" + reporterPath]);
      expect(this.fakeJasmine.clearReporters).not.toHaveBeenCalled();
      expect(this.fakeJasmine.addReporter).not.toHaveBeenCalled();
    ...
    it("should not configure stopping spec on expectation failure by default", function() {
      this.command.run(this.fakeJasmine, []);
      expect(
        this.fakeJasmine.stopSpecOnExpectationFailure
