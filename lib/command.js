  ...
  this.run = function(jasmine, commands) {
    ...
    Object.keys(subCommands).forEach(function(cmd) {
      ...
      if (commands.indexOf(cmd) >= 0) {
        ...
        commandObject.alias &&
        commands.indexOf(commandObject.alias) >= 0
      ) {
        commandToRun = commandObject;
      }
    });
...
function initJasmine(options) {
  ...
  if (!fs.existsSync(path.join(specDir, "support/jasmine.json"))) {
    fs.writeFileSync(
      path.join(specDir, "support/jasmine.json"),
      fs.readFileSync(
...
function help(options) {
  ...
  print(
    "%s\tfilter specs to run only those that match the given string",
    lPad("--filter=", 18)
  );
  print(
    "%s\tload helper files that match the given string",
    lPad("--helper=", 18)
  );
  print("%s\tload module that match the given string", lPad("--require=", 18));
  print(
    "%s\t[true|false] stop spec execution on expectation failure",
    lPad("--stop-on-failure=", 18)
  );
  print(
    "%s\t[true|false] stop Jasmine execution on spec failure",
    lPad("--fail-fast=", 18)
  );
  print("%s\tpath to your optional jasmine.json", lPad("--config=", 18));
  print(
    "%s\tpath to reporter to use instead of the default Jasmine reporter",
    lPad("--reporter=", 18)
  );
  print("");
  print(
